...
import {
  CheckCircle2, Circle, Clock, Rocket, Zap, Shield,
  Smartphone, Globe, Users, Package, ChevronDown
} from "lucide-react";
...
const nodeColorClass = (phase) =>
  phase === "done"       ? "bg-emerald-400 shadow-[0_0_16px_rgba(52,211,153,0.7)]" :
  phase === "inprogress" ? "bg-violet-400  shadow-[0_0_16px_rgba(139,92,246,0.7)]" :
  "bg-white/20";
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] uppercase tracking-widest text-white/30 leading-none mb-1">{milestone.quarter}</p>
            <p className="text-base font-semibold text-white leading-tight">{milestone.title}</p>
          </div>
        </div>
        <span className={`flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border flex-shrink-0 ${ph.textColor} ${ph.bg} ${ph.border}`}>
        ...
        {milestone.items.map((item) => {
          ...
          return (
            <li key={item.text} className="flex items-start gap-2.5">
              ...
              <span className={`text-sm leading-snug ${item.status === "planned" ? "text-white/30" : "text-white/65"}`}>{item.text}</span>
            </li>
          );
        })}
      </ul>
      ...
      >
        ...
        <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.25 }} className="flex items-center">
            ...
            <dl className="mt-4 space-y-3">
              {milestone.details.map((d) => (
                <div key={d.label} className="flex flex-col gap-0.5">
                  <dt className="text-[10px] uppercase tracking-widest text-white/25 font-medium">{d.label}</dt>
...
const RoadmapPage = () => {
  ...
  const STATUS = {
    done:       { label: t("roadmap.shipped"),    textColor: "text-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20", Icon: CheckCircle2, statusMap: {} },
    inprogress: { label: t("roadmap.inProgress"), textColor: "text-violet-400",  bg: "bg-violet-500/10",  border: "border-violet-500/20",  Icon: Clock,        statusMap: {} },
    planned:    { label: t("roadmap.planned"),    textColor: "text-white/30",    bg: "bg-white/5",        border: "border-white/10",       Icon: Circle,       statusMap: {} },
  };
  const statusMap = {
  ...
  Object.values(STATUS).forEach((s) => { s.statusMap = statusMap; });
    ...
    <PageWrapper>
      ...
      <main className="pt-28 pb-24 px-4 sm:px-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.28 }}
          ...
          <StatPill value={doneCount}  label={t("roadmap.shipped")}    color="bg-emerald-500/10 border-emerald-500/20 text-emerald-400" />
          <StatPill value={ipCount}    label={t("roadmap.inProgress")} color="bg-violet-500/10  border-violet-500/20  text-violet-400"  />
          <StatPill value={planCount}  label={t("roadmap.planned")}    color="bg-white/5        border-white/10       text-white/30"    />
        </motion.div>
