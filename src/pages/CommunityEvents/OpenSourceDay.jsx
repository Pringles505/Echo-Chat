...
import { Github, Code, Star, GitPullRequest, Calendar, MapPin, Clock, Users, Zap, Award } from 'lucide-react';
      ...
      <main className="relative pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-20">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-xl text-zinc-400 max-w-2xl mx-auto leading-relaxed mb-8">
              A full day dedicated to pushing Echo's open source projects forward together. Pick an issue, write code, get merged.
            </p>
            <div className="flex flex-wrap items-center justify-center gap-6 text-zinc-500 text-sm mb-10">
              <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-emerald-400" /> July 4, 2026</span>
              <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-emerald-400" /> 10:00 – 18:00 CET</span>
              <span className="flex items-center gap-2"><Github className="w-4 h-4 text-emerald-400" /> Remote · GitHub</span>
            </div>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <a href="https://github.com/echo-chat-protocol" target="_blank" rel="noreferrer"
        ...
        <div className="mb-24">
              ...
              {filters.map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={`px-4 py-2 rounded-full text-sm font-bold transition-all ${
                    filter === f ? 'bg-emerald-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-white border border-white/10'
                  }`}>
                  {f}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((proj, i) => (
              <motion.div key={proj.name} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.06 }}
                whileHover={{ y: -4 }}
                className="p-6 rounded-2xl bg-zinc-900/40 border border-white/5 hover:border-emerald-500/30 transition-colors">
                <div className="flex items-start justify-between mb-4">
                  <span className={`text-xs font-bold px-3 py-1 rounded-full border ${proj.tagColor}`}>{proj.tag}</span>
                  <span className={`text-xs font-bold ${difficultyColors[proj.difficulty]}`}>{proj.difficulty}</span>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-zinc-500 border-t border-white/5 pt-4">
                  ...
                  <span className="flex items-center gap-1"><Code className="w-3.5 h-3.5" /> {proj.issues} open issues</span>
        ...
        <div className="text-center">
          <Link to="/community" className="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 text-sm font-medium transition-colors">
            ← Back to Community
          </Link>
        </div>
      </main>
