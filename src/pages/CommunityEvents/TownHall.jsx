      ...
      <main className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-20">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-bold mb-6 tracking-tight">
              {t('townHall.title')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">{t('townHall.titleHighlight')}</span>
            </h1>
          ...
          <div className="bg-zinc-900/50 border border-emerald-500/30 rounded-3xl p-8 md:p-12 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-center">
              <div className="flex-1">
                <h2 className="text-2xl font-bold mb-2 text-emerald-400">{t('townHall.nextEvent.title')}: {t('townHall.nextEvent.date')}</h2>
                <h3 className="text-4xl font-bold text-white mb-4">{featuredTopics[0].title}</h3>
                <p className="text-zinc-400 mb-6">
                  {featuredTopics[0].desc}
                </p>
                <div className="flex items-center gap-4">
                  <div className="flex -space-x-3">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="w-10 h-10 rounded-full bg-zinc-800 border-2 border-black flex items-center justify-center text-xs font-bold">
                        U{i}
                      </div>
                    ))}
                  </div>
                  <span className="text-sm text-zinc-500">+450 {t('communityPage.events.attending')}</span>
                </div>
              </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-8">
            ...
            <a href="#" className="text-emerald-400 hover:text-emerald-300 flex items-center gap-2 text-sm font-bold">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              >
                ...
                <h3 className="font-bold text-lg mb-1 group-hover:text-emerald-400 transition-colors">{video.title}</h3>
