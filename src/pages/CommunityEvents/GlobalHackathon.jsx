...
const GlobalHackathon = () => {
  ...
  const tracks = [
    { icon: Lock, title: t('globalHackathon.tracks.privacy.title'), desc: t('globalHackathon.tracks.privacy.desc'), color: 'text-purple-400' },
    { icon: Globe, title: t('globalHackathon.tracks.decentralization.title'), desc: t('globalHackathon.tracks.decentralization.desc'), color: 'text-cyan-400' },
    { icon: Users, title: t('globalHackathon.tracks.social.title'), desc: t('globalHackathon.tracks.social.desc'), color: 'text-green-400' },
  ];
      ...
      <main className="relative pt-32 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-24">
          ...
          <h1 className="text-6xl md:text-8xl font-black mb-6 tracking-tighter uppercase">
            {t('globalHackathon.title')} <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">{t('globalHackathon.titleHighlight')}</span>
          </h1>
        ...
        {/* Tracks */}
        <div className="mb-24">
          <h2 className="text-3xl font-bold mb-10 text-center">{t('globalHackathon.tracks.title')}</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {tracks.map((track, i) => (
              ...
              >
                <div className={`w-12 h-12 rounded-lg bg-white/5 flex items-center justify-center mb-6 ${track.color}`}>
          ...
          <div className="relative z-10 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end">
              ...
              <div className="order-1 md:order-2 p-8 rounded-2xl bg-zinc-800/50 border border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.1)] transform md:-translate-y-4">
                <div className="text-xl font-bold text-cyan-400 mb-2 uppercase tracking-widest">{prizes.first.title}</div>
                <div className="text-6xl font-black text-white mb-6">{prizes.first.amount}</div>
                <ul className="text-zinc-300 space-y-3">
                  <li className="flex items-center justify-center gap-2"><Trophy className="w-4 h-4 text-yellow-500" /> {prizes.first.perks[0]}</li>
        ...
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl font-bold mb-10 text-center">{t('globalHackathon.timeline.title')}</h2>
          <div className="space-y-8 border-l-2 border-white/10 pl-8 ml-4">
            {timelineItems.map((item, i) => (
              <div key={i} className="relative">
                ...
                <div className="text-cyan-400 font-mono text-sm mb-1">{item.day} • {item.time}</div>
