...
const speakers = [
  { name: 'Dr. Alice Chen', role: 'Cryptography Researcher, MIT', topic: 'Post-Quantum Protocols', avatar: '👩‍🔬', featured: true },
  { name: 'Marco V.', role: 'Founder, Echo', topic: 'Echo v3.0 Vision', avatar: '🧑‍💻', featured: true },
  { name: 'Lena Hoffmann', role: 'Security Engineer, Open Privacy', topic: 'Zero-Knowledge in Production', avatar: '👩‍💻', featured: true },
  { name: 'James Park', role: 'Protocol Lead, Signal', topic: 'The Double Ratchet Today', avatar: '🕵️', featured: false },
  { name: 'Sara Okonkwo', role: 'Backend Engineer, Echo', topic: 'Scaling Encrypted Infra', avatar: '👩‍🏫', featured: false },
  { name: 'Dmitri Volkov', role: 'Open Source Advocate', topic: 'Community Governance Models', avatar: '🧑‍🏫', featured: false },
];

const agenda = [
  { day: 'Day 1 – Jun 10', time: '09:00', title: 'Opening Keynote & Echo v3 Announcement', speaker: 'Marco V.' },
  { day: 'Day 1 – Jun 10', time: '10:30', title: 'Post-Quantum Cryptography Deep Dive', speaker: 'Dr. Alice Chen' },
  { day: 'Day 1 – Jun 10', time: '12:00', title: 'Lunch & Networking' },
  { day: 'Day 1 – Jun 10', time: '14:00', title: 'Zero-Knowledge Proofs in Real Products', speaker: 'Lena Hoffmann' },
  { day: 'Day 1 – Jun 10', time: '16:00', title: 'Open Source Panel Discussion', speaker: 'All Speakers' },
  { day: 'Day 2 – Jun 11', time: '09:00', title: 'Scaling End-to-End Encrypted Infrastructure', speaker: 'Sara Okonkwo' },
  { day: 'Day 2 – Jun 11', time: '10:30', title: 'The Double Ratchet Algorithm: Past & Future', speaker: 'James Park' },
  { day: 'Day 2 – Jun 11', time: '12:00', title: 'Lunch & Networking' },
  { day: 'Day 2 – Jun 11', time: '14:00', title: 'Community Governance Workshop', speaker: 'Dmitri Volkov' },
      ...
      <main className="relative pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-24">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <h1 className="text-6xl md:text-8xl font-black mb-4 tracking-tighter">
              ECHO<span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400">CON</span>
            </h1>
            ...
            <div className="flex items-center justify-center gap-6 text-zinc-500 text-sm mt-6 mb-10">
              <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-violet-400" /> June 10–11, 2026</span>
              <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-violet-400" /> Madrid, Spain + Live Stream</span>
              <span className="flex items-center gap-2"><Clock className="w-4 h-4 text-violet-400" /> 2 Days</span>
            </div>
        ...
        <div className="mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {speakers.filter(s => s.featured).map((s, i) => (
              <motion.div key={i} whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 300 }}
                ...
                <p className="text-violet-400 text-xs font-bold uppercase tracking-wider mb-3">{s.role}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {speakers.filter(s => !s.featured).map((s, i) => (
              <div key={i} className="flex items-center gap-4 p-5 rounded-2xl bg-zinc-900/30 border border-white/5">
        ...
        <div className="mb-24">
            ...
            {['Day 1 – Jun 10', 'Day 2 – Jun 11'].map(day => (
              <button key={day} onClick={() => setActiveDay(day)}
                className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${
                  activeDay === day ? 'bg-violet-600 text-white' : 'bg-zinc-900 text-zinc-400 hover:text-white border border-white/10'
                }`}>
                {day}
              </button>
            ))}
          ...
          <div className="space-y-3">
            {agenda.filter(a => a.day === activeDay).map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.06 }}
                ...
                <div className="w-16 text-sm font-mono text-violet-400 flex-shrink-0">{item.time}</div>
                <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <p className="text-zinc-400 mb-6 leading-relaxed">
                EchoCon 2026 takes place at the <span className="text-white font-semibold">Espacio Rastro</span>, a modern event space in the heart of Madrid — with a full live stream for remote attendees.
              </p>
              <div className="space-y-3 text-sm text-zinc-400">
                <div className="flex items-center gap-3"><MapPin className="w-4 h-4 text-violet-400" /> Calle de la Ribera de Curtidores, 12, Madrid</div>
                <div className="flex items-center gap-3"><Calendar className="w-4 h-4 text-violet-400" /> June 10–11, 2026 · Doors open 08:30</div>
                <div className="flex items-center gap-3"><Globe className="w-4 h-4 text-violet-400" /> Free live stream available worldwide</div>
              </div>
            </div>
