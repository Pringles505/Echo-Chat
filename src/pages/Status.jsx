...
import {
  ...
  RefreshCw, Server, Globe, Zap, Database, Lock, Bell
} from "lucide-react";
...
const CFG_META = {
  operational: { Icon: CheckCircle2, color: "text-emerald-400", dot: "bg-emerald-400", bg: "bg-emerald-500/10", border: "border-emerald-500/20" },
  degraded:    { Icon: AlertTriangle, color: "text-yellow-400",  dot: "bg-yellow-400",  bg: "bg-yellow-500/10",  border: "border-yellow-500/20"  },
  outage:      { Icon: XCircle,       color: "text-red-400",     dot: "bg-red-400",     bg: "bg-red-500/10",     border: "border-red-500/20"     },
  maintenance: { Icon: Wrench,        color: "text-blue-400",    dot: "bg-blue-400",    bg: "bg-blue-500/10",    border: "border-blue-500/10"    },
};
...
function useStatuses(serviceCount) {
  ...
  const randomize = useCallback(() => {
    ...
    setStatuses(Array.from({ length: serviceCount }, () => pool[Math.floor(Math.random() * pool.length)]));
  }, [serviceCount]);
  useEffect(() => { randomize(); const id = setInterval(randomize, 30000); return () => clearInterval(id); }, [randomize]);
...
export default function StatusPage() {
  ...
  const handleRefresh = () => { refresh(); setLastChecked(new Date()); };
    ...
    <PageWrapper>
      ...
      <main className="pt-28 pb-24 px-4 sm:px-6 max-w-4xl mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}
          ...
          <div>
            <h1 className="text-3xl font-bold text-white tracking-tight">{t("statusPage.pageTitle")}</h1>
            <p className="text-sm text-white/35 mt-1">
              {t("statusPage.lastChecked")} <span className="text-white/55">{fmt(lastChecked)}</span>
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border ${summary.bg} ${summary.border}`}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
          ...
          {services.map((svc, i) => {
            ...
            return (
              <div key={i} className={`flex items-center gap-4 px-5 py-4 ${i < services.length - 1 ? "border-b border-white/[0.06]" : ""}`}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.2 }}>
          <h2 className="text-lg font-semibold text-white mb-5">{t("statusPage.incidentHistory")}</h2>
              ...
              return (
                <div key={i} className="bg-white/[0.04] border border-white/10 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full border ${colorCls}`}>
                      {inc.type}
                    </span>
                  </div>
