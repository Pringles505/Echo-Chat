...
export async function applyUpdatePath(treeNodes, updatePath, senderLeafIndex, leafCount, myLeafIndex, myPrivKeyB64) {
...
export async function buildAddCommit({ state, newMember, memberInitKeys }) {
  ...
  const senderRosterEntry = newRoster.find(m => String(m.userId) === String(currentState.selfUserId));
...
export async function applyCommit({ state, commit, myInitPrivKeyB64 }) {
  ...
  const senderEntry = normalizeRoster(currentState.roster)
    .find(m => m.leafIndex === commit.senderLeafIndex);
