...
describe("dr.js util exports", () => {
  ...
  it("continueDoubleRatchetChain derives receivingChainKey/newRootKey from hkdf_derive output", async () => {
    ...
    expect(Buffer.from(out.receivingChainKey)).toEqual(Buffer.from(new Uint8Array(32).fill(1)));
    expect(Buffer.from(out.newRootKey)).toEqual(Buffer.from(new Uint8Array(32).fill(2)));
    ...
    const [_ikm, salt, info, len] = dh.hkdf_derive.mock.calls[0];
  ...
  it("initializeDoubleRatchetResponse returns a root_key and pins identity when no saved peer exists", async () => {
    ...
    const out = await initializeDoubleRatchetResponse({}, message, "bob", new Uint8Array(32).fill(4));
  ...
  it("initializeDoubleRatchetResponse throws PEER_IDENTITY_CHANGED if stored identity differs", async () => {
    ...
    km.getIdentityKeys.mockResolvedValue({ privatePreKey: toB64(new Uint8Array(32).fill(1)) });
    km.getPeerIdentityKeys.mockResolvedValue({ publicIdentityKeyX25519: saved, publicIdentityKeyEd25519: null });
    api.fetchPublicIdentityKeyX25519.mockResolvedValue(fetched);
    api.fetchPublicIdentityKeyEd25519.mockResolvedValue(null);

    await expect(
      initializeDoubleRatchetResponse({}, { publicEphemeralKey: toB64(new Uint8Array(32).fill(3)) }, "bob", new Uint8Array(32).fill(4))
