...
const dhWasm = vi.hoisted(() => ({
  ...
  generate_public_ephemeral_key: vi.fn(async (priv) => new Uint8Array(32).fill(((new Uint8Array(priv)[0] + 1) & 0xff))),
...
const dr = vi.hoisted(() => ({
  ...
  continueDoubleRatchetChain: vi.fn(async (_socket, _targetUserId, pubBase64, priv, rootKey) => {
    ...
    return { receivingChainKey: okm.slice(0, 32), newRootKey: okm.slice(32) };
  }),
}));
...
describe("protocol minimum (transcript) tests", () => {
  ...
  it("4) DH ratchet transcript: receiving new DH triggers ratchet; next send uses new DH", async () => {
    const { A_dh_pub_b64, B_dh_pub_b64 } = seedConversation({ a: "alice", b: "bob" });
    ...
    const bobSendingSeed = okm.slice(0, 32);
